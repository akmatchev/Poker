...
print_meta "Checking for Git repo"

if [[ ! -d ".git" ]]; then
  print_fatal "Git repo not found. Did you initialize git in this folder?"
  exit 1
fi
...
if [[ $? -ne 0 ]]; then
  print_fatal "You have not set [hours_worked] in author.ml."
  exit 1
fi
